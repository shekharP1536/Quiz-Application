...
function showQuestions(index) {
    ...
    let creator_tag = document.querySelector('.creator');
    // Creating a new span for the question and adding the question text
    let que_tag = '<span>' + questions[index].numb + ". " + questions[index].question + '</span>';

    let creator = '<span>Created by <strong>' + questions[index].creator + "</strong></span>"
    // Adding an image if the question object has an 'image' property
    let image_tag = "";
    if (questions[index].image) {
      image_tag = '<img src="' + questions[index].image + '" alt="Question Image" class="question-image">';
    }
...
function showResult(){
    ...
    if (userScore > 3){ // if user scored more than 3
        ...
        let scoreTag = '<span>and congrats! 🎉, You attendend <p>'+ userScore +'</p> out of <p>'+ questions.length +'</p></span>';
        scoreText.innerHTML = scoreTag;  //adding new span tag inside score_Text
    }
    else if(userScore > 1){ // if user scored more than 1
        let scoreTag = '<span>and nice 😎, You attendend <p>'+ userScore +'</p> out of <p>'+ questions.length +'</p></span>';
        scoreText.innerHTML = scoreTag;
    }
    else{ // if user scored less than 1
        let scoreTag = '<span>and sorry 😐, You attendend only <p>'+ userScore +'</p> out of <p>'+ questions.length +'</p></span>';
        scoreText.innerHTML = scoreTag;
    }
}
